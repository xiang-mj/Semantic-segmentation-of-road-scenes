...
from utils.f_boundy import eval_mask_boundary
import datasets
import loss
import network
import optimizer
...
parser.add_argument('--arch', type=str, default='network.semantic_segmentation.Transformer_dual_branch',
                    help='Network architecture.')
parser.add_argument('--dataset', type=str, default='cityscapes',
                    help='cityscapes, camvid, ade20k')
...
parser.add_argument('--trunk', type=str, default='resnet-101',
                    help='trunk model, can be: resnet-101 (default), resnet-50')
...
parser.add_argument('--bs_mult_val', type=int, default=8,
...
def validate(val_loader, net, criterion, optim, curr_epoch, writer):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        inputs, gt_cuda = inputs, gt_image

        with torch.no_grad():
            output = net(inputs)[0]  # output = (1, 19, 713, 713)
        # print(output.size())#torch.Size([19, 1024, 2048])
        # print(args.dataset_cls.num_classes)#19
        #print(gt_image.size())##torch.Size([1, 1024, 2048])

        assert output.size()[2:] == gt_image.size()[1:]
        assert output.size()[1] == args.dataset_cls.num_classes

        #print(output.size())#torch.Size([19, 1024, 2048])
        #print(gt_cuda.size())#torch.Size([1, 1024, 2048])
