...
def get_loss(args):
    ...
    criterion_val = CrossEntropyLoss2d_val(size_average=True,
                                       weight=None,
                                       ignore_index=args.dataset_cls.ignore_label).cuda()
    return criterion, criterion_val
...
class CrossEntropyLoss2d(nn.Module):
    ...
    def forward(self, inputs, targets):
        return self.nll_loss(F.log_softmax(inputs[0]), targets)
class CrossEntropyLoss2d_val(nn.Module):
    """
    Cross Entroply NLL Loss
    """

    def __init__(self, weight=None, size_average=True, ignore_index=255):
        super(CrossEntropyLoss2d_val, self).__init__()
        logging.info("Using Cross Entropy Loss")
        self.nll_loss = nn.NLLLoss2d(weight, size_average, ignore_index)
        # self.weight = weight
